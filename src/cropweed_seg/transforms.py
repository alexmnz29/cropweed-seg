...
def build_transforms(split: str, crop_size: int = DEFAULT_CROP_SIZE) -> v2.Compose:
    """Return the transform pipeline for a given split.

    train: random crop of crop_size -> float scale -> ImageNet normalize
    val/test: float scale -> ImageNet normalize (full frame, no crop)
    crop_size only affects the train split.
    """
